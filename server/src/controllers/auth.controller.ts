...
import { APIErrorResponse, APIResponse, APISuccessResponse } from "typings/response";
...
export default class Authcontroller extends AuthControllerInterface {
   ...
   */
    ...
    public static async signup(req: TypedRequest,
        res: TypedResponse<APIResponse<APISuccessResponse<{name: string, email: string, password: string | number}> | APIErrorResponse | void>>,
        next: TypedNextFn)  {
            try {
     ...
     */
    public static async signin<T>(req: TypedRequest, res: TypedResponse<T>, next: TypedNextFn) {
        try {
