...
const VerifyModal: React.FC<IVerifyModalProps> = ({ toggleModal, id }) => {
  const toast = useCustomToast();
  const { verifyUser, loading, users } = useAdminStore();
  const found = users?.find((item) => item?.id === id);
  console.log(found);
  const handleVerify = async () => {
    if (id) {
      await verifyUser(id).then((res) => toast(res.message, "success"));
      toggleModal();
    } else {
        ...
        <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
          ...
          <div>
            <div className="text-white text-xl py-2 text-center mb-4">
              do you want to approve and verify {found?.email}?
            </div>
            <div className="flex items-center space-x-4">
              ...
              >
                {loading ? <Spinner /> : "Verify User Account"}
              </button>
              <button
                type="button"
